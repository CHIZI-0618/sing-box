...
| `shared.data_plane: packet_rewrite` | Not supported for shared clients |
...
interface to answer from. `shared.data_plane: packet_rewrite` uses the separate
`SharedNetworkBackend`, which does not attach the FakeIP ICMP responder.
Enabling `reply` when these are the only enabled paths is refused at startup.

Support is path-specific when local and shared interception are combined:

- `local: tc` plus `shared: packet_rewrite` is accepted, but only local TC
  traffic is answered. Shared packet-rewrite clients receive no FakeIP ICMP
  reply.
- `local: cgroup` plus `shared: socket_assign` is accepted, but only shared
  clients are answered. Traffic generated by processes in the local cgroup
  receives no FakeIP ICMP reply.

Use `local.data_plane: tc` for local replies and
