...
!!! warning "Linux 6.6 LPM trie 兼容性"
    ...
    不受影响。UID 范围、`bypass_rule_set` 和 shared 来源 CIDR 筛选需要
    Linux 6.6.47，或包含上游修复
...
控制 TCP 和 UDP 目标端口 53 如何与绕过策略交互，可选值为：

| 值 | 行为 |
|----|------|
| `hijack` | 在私网和规则集绕过判断前拦截目标端口 53。 |
| `respect_bypass_hijack` | 先对目标端口 53 执行私网和规则集绕过判断，再决定是否拦截。 |
| `off` | 目标端口 53 保持系统原有路径。 |

默认值为 `hijack`。

该策略同时作用于本机 cgroup 和 `shared_network`。热点通常应保留
`hijack`，除非主机已经提供独立 DNS 服务。它只负责捕获 53 端口，
不等同于 `hijack-dns` 路由动作。
对于 `shared_network`，`respect_bypass_hijack` 还会执行本机地址绕过判断。
...
包名策略匹配内核记录在每个 socket 上的 UID，只保证直接以该包 UID 创建的 socket。
由系统 DNS resolver、`DownloadManager`、WebView 或其他 isolated process、SDK
sandbox 等组件代应用创建的流量可能使用其他 UID，不保证遵循该包名规则。

本机 cgroup 启动日志会输出包名和 Android 用户展开后的最终 `include`、`exclude`
UID ranges 以及 `include_configured`。可据此确认实际写入 eBPF map 的策略。
