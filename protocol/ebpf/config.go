...
func normalizeDNSMode(mode string) (string, error) {
	...
	case dnsModeRespectBypassHijack, dnsModeOff:
		return mode, nil
	default:
...
func validateSharedNetworkProtocols(options option.EBPFSharedNetworkOptions, enableUDP bool, dnsMode string) error {
	if options.Enabled && dnsMode != dnsModeOff && !enableUDP {
		return E.New("shared_network with DNS interception requires UDP")
	}
	return nil
}
