...
func TestSharedNetworkDataPathIntegration(t *testing.T) {
	...
	if repairedAttachment.ingress.Id != attachment.ingress.Id {
		t.Fatalf("shared-network ingress program changed during repair: %d != %d", repairedAttachment.ingress.Id, attachment.ingress.Id)
	}
