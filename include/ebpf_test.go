...
func TestEBPFInboundPolicyOptions(t *testing.T) {
	...
	if err := json.UnmarshalContext(ctx, []byte(`{
		...
		"local": {
			...
			"bypass_rule_set": ["geoip-cn", "geoip-private"]
		}
	}`), &inboundOptions); err != nil {
		t.Fatal(err)
	}
	...
	if len(ebpfOptions.Local.BypassRuleSet) != 2 || ebpfOptions.Local.BypassRuleSet[0] != "geoip-cn" || ebpfOptions.Local.BypassRuleSet[1] != "geoip-private" {
