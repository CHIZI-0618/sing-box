...
Shared-network host addresses are stored in exact-match hash maps. They are
always individual IPv4 `/32` or IPv6 `/128` addresses, so an LPM trie would add
no matching capability. Keeping these maps exact also avoids the Linux 6.6.0
through 6.6.46 LPM-trie UBSAN bug. Policies that require real longest-prefix
matching are rejected on a known-unfixed kernel unless BTF shows that the
flexible-key fix has been backported.
