...
int sb_ebpf_shared_network_prepare(
    ...
    const struct sb_ebpf_map_spec maps[] = {
        ...
        {"stats", BPF_MAP_TYPE_ARRAY, sizeof(uint32_t), sizeof(uint64_t),
         SB_SHARED_STAT_COUNT, 0U, &runtime->stats_map_fd},
        {"original-to-token", BPF_MAP_TYPE_HASH, sizeof(struct sb_shared_original_key),
         sizeof(struct sb_shared_token_value), proxy_capacity, BPF_F_NO_PREALLOC,
        ...
        {"reply lookup", BPF_MAP_TYPE_HASH, sizeof(struct sb_shared_reply_key),
         sizeof(struct sb_shared_reply_value), proxy_capacity, BPF_F_NO_PREALLOC,
         &runtime->reply_map_fd},
        {"listener lookup", BPF_MAP_TYPE_HASH, sizeof(struct sb_shared_listener_key),
         sizeof(struct sb_shared_original_value), proxy_capacity, BPF_F_NO_PREALLOC,
        ...
        {"host IPv4", BPF_MAP_TYPE_HASH, sizeof(struct sb_ebpf_ipv4_cidr_lpm_key),
         sizeof(uint8_t), 256U, 0U, &runtime->host_ipv4_map_fd},
        {"host IPv6", BPF_MAP_TYPE_HASH, sizeof(struct sb_ebpf_ipv6_cidr_lpm_key),
         sizeof(uint8_t), 256U, 0U, &runtime->host_ipv6_map_fd},
