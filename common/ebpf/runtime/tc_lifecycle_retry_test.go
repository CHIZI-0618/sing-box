...
func TestTCRuntimeCloseRetriesOwnedResources(t *testing.T) {
	failed := &tcRetryResource{fail: true}
	a := newTestTCAttachment(t, "runtime", testStartupLockIndex+0x104)
	a.localICMPLink = failed
	released := &tcRetryResource{}
	...
	if d.IsClosed() || d.backend == nil || d.routing == nil || d.delivery == nil || len(d.attachments) != 1 || d.attachments[0] != a || routingLock.calls != 0 {
		t.Fatal("runtime lost failed owner or its dependencies")
	}
	failed.fail = false
	if err := d.Close(); err != nil {
		t.Fatal(err)
	}
	if !d.IsClosed() || tcLockHeld(t, a.interfaceIndex) {
		t.Fatal("second runtime Close did not finish")
	}
	if err := d.Close(); err != nil {
		t.Fatal(err)
	}
...
func TestTCStartupAttachFailureReturnsRetryOwner(t *testing.T) {
	...
	if err := d.Close(); err == nil || d.IsClosed() {
		t.Fatal("startup cleanup dropped data plane resources")
	}
	failed.fail = false
	if err := d.Close(); err != nil {
		t.Fatal(err)
